...
static int rfkill_set_power(void *data, bool blocked)
{
	...
	AP6210_INFO("rfkill set power %s\n", ( blocked ? "blocked" : "unblocked" ));
...
static int sw_rfkill_probe(struct platform_device *pdev)
{
	...
	if (unlikely(!sw_rfkill)) {
		AP6210_ERR("Unable to alocate rfkill structure.\n" );
		return -ENOMEM;
		}

	ret = rfkill_register(sw_rfkill);
	if (unlikely(ret)) {
		AP6210_ERR("Unable to register rfkill structure.\n" );
		rfkill_destroy(sw_rfkill);
	}
	AP6210_INFO("rfkill structure registered successfully.\n" );
	return ret;
}
...
int __init sw_rfkill_init(void)
{
	if (SCRIPT_PARSER_OK != script_parser_fetch("bt_para", "bt_used", &bt_used, 1)) {
		AP6210_ERR("parse bt_used failed in script.fex.\n" );
		return -1;
	}
	...
	AP6210_INFO("platform device(sunxi-rfkill) registered successfully.\n" );
